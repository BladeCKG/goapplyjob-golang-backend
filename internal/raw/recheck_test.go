...
func TestRecheckSkippableClearsRecoveringJobs(t *testing.T) {
	...
	_, err = db.SQL.ExecContext(context.Background(), `INSERT INTO raw_us_jobs (id, url, post_date, is_ready, is_skippable, is_parsed, retry_count, raw_json) VALUES
		(1, 'https://example.com/jobs/1', '2026-02-12T10:00:00Z', true, true, false, 0, NULL),
		(2, 'https://example.com/jobs/2', '2026-02-12T10:00:00Z', true, true, false, 0, NULL),
		(3, 'https://example.com/jobs/3', '2026-02-12T10:00:00Z', true, false, false, 0, NULL)`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if isReady || isSkippable {
		t.Fatalf("expected cleared row to reset, got is_ready=%t is_skippable=%t", isReady, isSkippable)
	}
	...
	if !isReady || !isSkippable {
		t.Fatalf("expected 404 row to stay skippable, got is_ready=%t is_skippable=%t", isReady, isSkippable)
	}
}
