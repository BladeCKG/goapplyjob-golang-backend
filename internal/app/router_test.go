...
func TestJobsFilterOptionsIncludesHierarchyMetadata(t *testing.T) {
	db := testDB(t, "test_filter_options_hierarchy")
	defer db.Close()
...
func TestJobsTechStackFilterAndOptions(t *testing.T) {
	db := testDB(t, "test_filter_options_tech_stack")
	defer db.Close()
...
func TestJobsEmploymentTypeFilterAndOptions(t *testing.T) {
	db := testDB(t, "test_filter_options_employment_type")
	defer db.Close()
...
func testRouter(t *testing.T) (*gin.Engine, *database.DB) {
	t.Helper()
	db := testDB(t, "test_page_extract")
	return testRouterFromDB(t, db), db
}

func testDB(t *testing.T, schemaName string) *database.DB {
	t.Helper()
	cfg := testConfig()
	cfg.DatabaseURL = testDatabaseURL(t, schemaName)
	db, err := database.Open(cfg.DatabaseURL)
	if err != nil {
		t.Fatal(err)
	}
...
func testRouterFromDB(t *testing.T, db *database.DB) *gin.Engine {
	t.Helper()
	cfg := testConfig()
	cfg.AuthDebugReturnCode = true
	cfg.AuthEnableCodeLogin = true
